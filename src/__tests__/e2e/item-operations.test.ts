...
describe('E2E Item Operations', () => {
    ...
    it('should check multiple items in batch', async () => {
        ...
        const dentistItem = items.find(
            (item) => item.content === 'Call dentist'
        );
    ...
    it('should verify item was edited', async () => {
        ...
        const butterItem = items.find(
            (item) => item.content === 'Buy organic butter'
        );
    ...
    it('should delete specific items in batch', async () => {
        ...
        const itemsToDelete = items.filter((item) =>
            item.content.includes('Item to delete')
        );
    ...
    it('should test moveItem functionality - move to top', async () => {
        ...
        const itemsBefore = await service.getItems(testListId);
        const secondItem = itemsBefore.find(
            (item) => item.content === 'Second item'
        );
        expect(secondItem).toBeDefined();

        // Move second item to top
        await service.moveItem(testListId, secondItem!.id, { position: 'top' });
    ...
    it('should test moveItem functionality - move to bottom', async () => {
        const itemsBefore = await service.getItems(testListId);
        const firstItem = itemsBefore.find(
            (item) => item.content === 'First item'
        );
        expect(firstItem).toBeDefined();

        // Move first item to bottom
        await service.moveItem(testListId, firstItem!.id, {
            position: 'bottom',
        });
    ...
    it('should test moveItem functionality - move before specific item', async () => {
        ...
        await service.moveItem(testListId, lastItem!.id, {
            before: firstItem!.id,
        });
    ...
    it('should test moveItem functionality - move after specific item', async () => {
        ...
        const itemsAfter = await service.getItems(testListId);
        const firstItemNewIndex = itemsAfter.findIndex(
            (item) => item.id === firstItem!.id
        );
        const secondItemNewIndex = itemsAfter.findIndex(
    ...
    it('should handle moveItem error cases', async () => {
        ...
        await expect(
            service.moveItem(testListId, firstItem!.id, {
